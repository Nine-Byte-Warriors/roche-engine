...
void Button_Widget::Draw( ID3D11Device* device, ID3D11DeviceContext* context, XMMATRIX worldOrtho, TextRenderer* textRenderer )
{
    ...
    // Button text
    XMVECTOR textsize = textRenderer->GetSpriteFont( FontSize::MEDIUM )->MeasureString( m_sText.c_str() );
    XMFLOAT2 textPos =
    {
    ...
    textRenderer->RenderString( m_sText, textPos, m_vTextColor, FontSize::MEDIUM, true );
}
