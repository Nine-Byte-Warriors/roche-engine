...
void Input_Widget::Draw( ID3D11Device* device, ID3D11DeviceContext* context, XMMATRIX worldOrtho, TextRenderer* textRenderer )
{
	...
	XMVECTOR textsize = textRenderer->GetSpriteFont( FontSize::MEDIUM )->MeasureString( m_sCurrText.c_str() );
	XMFLOAT2 textpos =
	{
	...
	textRenderer->RenderString( m_sCurrText, textpos, m_textColour, FontSize::MEDIUM, true );
}
